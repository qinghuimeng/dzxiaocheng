...
if(!isset($_GET['editsubmit'])) {
    ...
    if(empty($_GET['delete'])) {
        ...
        if(!$_G['setting']['editperdel']) {
            odz_error('post_edit_thread_ban_del', -2);
        }

        if($isfirstpost && $thread['replies'] > 0) {
            odz_error($thread['special'] == 3 ? 'post_edit_reward_already_reply' : 'post_edit_thread_already_reply', -3);
        }

        if($thread['special'] == 3) {
            if($thread['price'] < 0 && ($thread['dateline'] + 1 == $orig['dateline'])) {
                odz_error('post_edit_reward_nopermission', -5);
            }
        }

        if($rushreply) {
            odz_error('post_edit_delete_rushreply_nopermission', -4);
        }
    ...
    if($_G['forum_auditstatuson']) {
        ...
        if(!empty($_GET['delete']) && $isfirstpost) {
            ...
            if($isfirstpost && $modnewthreads) {
                ...
                odz_success('edit_newthread_mod_succeed');
            } elseif(!$isfirstpost && $modnewreplies) {
