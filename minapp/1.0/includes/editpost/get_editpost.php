...
if(empty($orig)) {
	...
	odz_error('post_edit_nopermission');
	...
	if(!$alloweditpost_status && $_G['group']['edittimelimit'] && TIMESTAMP - $orig['dateline'] > $_G['group']['edittimelimit'] * 60) {
		odz_error('post_edit_timelimit', -1, array('edittimelimit' => $_G['group']['edittimelimit']));
	}
}
...
if($special == 5) {
	...
	if($isfirstpost && $debate['umpirepoint'] && !$_G['forum']['ismoderator']) {
		odz_error('debate_umpire_comment_invalid');
	}
}
...
if(!isset($_GET['editsubmit'])) {
    ...
    if($postinfo['first'] && $threadsort) {
        require_once 'includes/functions/threadsort.php';
        $result['list'] = odz_threadsortshow_3($thread['sortid'], $_G['tid']);
    }
    ...
    // 安卓分类信息参数处理
    if(isset($_GET['typeoption']) && !is_array($_GET['typeoption'])) {
        if(defined('ODZ_CHARSET') && ODZ_CHARSET == 'gbk') {
            $_GET['typeoption'] = iconv('gbk', 'utf-8//IGNORE', $_GET['typeoption']);
        }
        $_GET['typeoption'] = odz_decode(json_decode($_GET['typeoption'], true));
    }
	...
	if(empty($_GET['delete'])) {
        ...
        odz_before_editpost($param, $modpost);
	...
	if($_G['forum_auditstatuson']) {
		if($audit == 1) {
            ...
            if(!empty($_GET['delete']) && $isfirstpost) {
                    odz_success('post_edit_delete_succeed');
                ...
                if($isfirstpost && $modpost->param('modnewthreads')) {
                        ...
                        showmessage('edit_newthread_mod_succeed');
                } elseif(!$isfirstpost && $modpost->param('modnewreplies')) {
                        C::t('forum_post')->update($thread['posttableid'], $pid, array('status' => 4), false, false, null, -2, null, 0);
                        updatemoderate('pid', $pid);
                        odz_success('edit_reply_mod_succeed');
                } else {
                        odz_success('post_edit_succeed');
                }
            }
        }
 ...
 * @author Paul
 */
function odz_before_editpost($parameters, $model) {
    global $_G;
    $sortid = $parameters['sortid'];
    $isfirstpost = $model->post['first'] ? 1 : 0;
    if($isfirstpost) {

        $parameters['typeid'] = isset($model->forum['threadtypes']['types'][$parameters['typeid']]) ? $parameters['typeid'] : 0;
        if(!$model->forum['ismoderator'] && !empty($model->forum['threadtypes']['moderators'][$model->thread['typeid']])) {
            $parameters['typeid'] = $model->thread['typeid'];
        }
        $parameters['sortid'] = isset($model->forum['threadsorts']['types'][$parameters['sortid']]) ? $parameters['sortid'] : 0;
        $typeexpiration = intval($_GET['typeexpiration']);

        if(!$parameters['typeid'] && $model->forum['threadtypes']['required'] && !$model->thread['special']) {
            showmessage('post_type_isnull');
        }

        if($model->forum['threadsorts']['types'][$sortid] && !$model->forum['allowspecialonly']) {
            require_once libfile('post/threadsorts', 'include');
            $_G['forum_optiondata'] = threadsort_validator($_GET['typeoption'], $model->post['pid']);
        }

        $model->param['threadimageaid'] = 0;
        $model->param['threadimage'] = array();

        if($model->forum['threadsorts']['types'][$parameters['sortid']] && $_G['forum_optiondata'] && is_array($_G['forum_optiondata'])) {
            ...
            foreach($_G['forum_optiondata'] as $optionid => $value) {
                ...
                if($_G['forum_optionlist'][$optionid]['type'] == 'image') {
                    // TODO minbbs 3.0 暂不支持编辑分类信息图片字段
                    continue;
                }
                ...
                C::t('forum_typeoptionvar')->update_by_tid($model->thread['tid'], array('value' => $value, 'sortid' => $parameters['sortid']), false, false, $optionid);
            }

            if($typeexpiration) {
                C::t('forum_typeoptionvar')->update_by_tid($model->thread['tid'], array('expiration' => (TIMESTAMP + $typeexpiration)), false, false, null, $parameters['sortid']);
            }

            if($sql || ($filedname && $valuelist)) {
                if(C::t('forum_optionvalue')->fetch_all_tid($parameters['sortid'], "WHERE tid='".$model->thread['tid']."'")) {
                    if($sql) {
                        C::t('forum_optionvalue')->update($parameters['sortid'], $model->thread['tid'], $model->forum['fid'], $sql);
                    }
                } elseif($filedname && $valuelist) {
                    C::t('forum_optionvalue')->insert($parameters['sortid'], "($filedname, tid, fid) VALUES ($valuelist, '".$model->thread['tid']."', '".$model->forum['fid']."')");
                }
            }
        }
    }
