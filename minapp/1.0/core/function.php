...
function avatar($uid, $size = 'middle', $returnsrc = FALSE, $real = FALSE, $static = FALSE, $ucenterurl = '') {
	...
	if(!$staticavatar && !$static) {
		...
		return $returnsrc ? $ucenterurl.'/avatar.php?uid='.$uid.'&size='.$size.'&random='.time() : '<img class="discuss_portrait" onerror="this.onerror=null;this.src=\'/'.$_G['minbbs_config']['minbbs_type'].'/assets/images/default.gif\';" src="'.$ucenterurl.'/avatar.php?uid='.$uid.'&size='.$size.($real ? '&type=real' : '').'" />';
        ...
        $error_image = isset($_G['minbbs_config']['default_image_url']) ? $_G['minbbs_config']['default_image_url'] : '';

		return $returnsrc ? $file : '<img onerrors="'.$error_image.'" src="'.$file.'" class="discuss_portrait" onerror="this.onerror=null;this.src=\''.$ucenterurl.'/images/noavatar_'.$size.'.gif\'" />';
...
function dsign($str, $length = 16){
	...
	require DISCUZ_ROOT.'source/discuz_version.php';



	if(DISCUZ_RELEASE < 20140101){
...
//		$s = gmdate('Y-n-j', $timestamp);
		...
		if($timestamp >= $todaytimestamp) {
			...
			if($time > 3600) {
				...
				return $time.($api === true ? ' ' :'&nbsp;').$lang['sec'].$lang['before'];
...
//	return dshowmessage($message, $url_forward, $values, $extraparam, $custom);
    ...
    require_once 'includes/functions/message.php';

    return dshowmessage($message, $url_forward, $values, $extraparam, $custom);

}

function view_showmessage($message, $values=array()) {
	$string = <<<EOF
<!DOCTYPE html>
<html>
<head>
...
.wrapper {}
.content { background-color:#f7f7f7; text-align:center; padding:8px 0; color:#666666; font-size:14px; }
</style>
</head>
<body>
<div class="wrapper">
...
function hideview() {
	var json = '{"hidview":1}';
	if(minbbsMobile.Android()) {
		window.shareInfo.jiaohu(json);
	}
	if(minbbsMobile.iOS()) {
		window.location.href = "objc://jiaohu:/" + json;
	}
}
...
EOF;
	...
	$string = str_replace("{CHARSET}", CHARSET, $string);
	$string = str_replace("{TITLE}", lang('template', 'board_message'), $string);
	$string = str_replace("{MESSAGE}", odz_lang($message, $values), $string);
...
function view_pwd_showmessage($message=false) {

    

     $html = '';

     $html.=  "<script type='text/javascript' src='assets/js/jquery-1.8.3.min.js'></script>
                ...
                <script type='text/javascript'>
                    ...
                    $(function() {
                        ...
                        var cmd = 'needPassword';

                        if(minbbsMobile.Android()) {

                            window.shareInfo.needPassword();
     ...
     if($message){
            ...
            $html.= '<div style="background-color:#f7f7f7;text-align:center;padding:5px;color:#666666;">'.odz_lang('passwd_incorrect', array()). '</div>';
...
function cknewuser($return=0) {
	...
	// 	if(empty($return)) showmessage('minbbs_no_privilege_avatar', '', array(), array());
