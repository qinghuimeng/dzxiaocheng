...
class discuz_application extends discuz_base{
    ...
    private function _init_input() {
        ...
        $param = trim(file_get_contents('php://input'));
        if($param){
                // 接口参数解密
                require_once 'includes/Mcrypt3Des.php';
                $e = new Mcrypt3Des();
                $param = $e->dencrypt($param);
                // emoji表情检测
                checkemoji($param) && define('EMOJI_EXISTS', 1);

                if(false !== $param) {
                    $param = json_decode($param, true);
                    $param = odz_decode($param);
                } else {
                    $param = array();
                }

                $_GET = array_merge($_GET, $param);
                $this->var['pa_count'] = count($param);
        }else{
            // emoji表情检测
            checkemoji(serialize($_GET)) && define('EMOJI_EXISTS', 1);

            $_GET = odz_decode($_GET);
            $this->var['pa_count'] = count($_GET);
        }
        unset($param);

        // if(isset($_GET['numsize'])&&$_GET['numsize']>0){ 
        //     foreach($_FILES as $key => $file){
        //         if($file['error'] != 0){
        //             exit;
        //         }
        //         if(move_uploaded_file($file['tmp_name'], DISCUZ_ROOT.'data/attachment/temp/'.time().$key.'.tmp')){
        //              $_GET['filedata'][]= DISCUZ_ROOT.'data/attachment/temp/'.time().$key.'.tmp';
        //         }
        //     }
        //     $_GET['upload_files'] = true;
        // }
    ...
    private function _init_config() {
        ...
        if(defined('MINBBS_DEBUG')){
            ini_set('display_errors', 'on');
            error_reporting(E_ALL);
        }

        if($_config['output']['staticurl']) $_config['output']['staticurl']='../../static/';//minbbs陆脫驴脷虏鹿露隆
    ...
    private function _init_user() {
        if($this->init_user) {
            if (!isset($_GET['auth']) && isset($_SERVER['HTTP_MINBBSAUTH'])) {
                $_GET['auth'] = trim($_SERVER['HTTP_MINBBSAUTH']);
            }
            if (isset($_GET['auth'])) {
            ...
            $mods = array('avatar', 'editpost', 'newthread', 'newreply', 'topicadmin', 'friends', 'forum_misc', 'bind');
            if(!empty($_GET['auth']) && in_array($_REQUEST['mod'], $mods)) {   

                if(!$discuz_uid) {
                    showmessage('token_expire');
                }

                if(empty($user)) {
                    showmessage('user_not_exists');
                }
