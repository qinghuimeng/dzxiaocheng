...
class table_home_praise extends discuz_table
{
    
	public function __construct() {

		$this->_table = 'home_praise';
		$this->_pk    = 'praid';

		parent::__construct();
	}
 
	public function fetch_all_by_uid_idtype($uid, $idtype, $praid = 0, $start = 0, $limit = 0) {
		$parameter = array($this->_table);
		$wherearr = array();
		if($praid) {
			$parameter[] = dintval($praid, is_array($praid) ? true : false);
			$wherearr[] = is_array($praid) ? 'praid IN(%n)' : 'praid=%d';
		}
		$parameter[] = $uid;
		$wherearr[] = "uid=%d";
		if(!empty($idtype)) {
	...
	public function count_by_uid_idtype($uid, $idtype, $praid = 0) {
		$parameter = array($this->_table);
		$wherearr = array();
		if($praid) {
			$parameter[] = dintval($praid, is_array($praid) ? true : false);
	...
	public function count_by_id_idtype($id, $idtype) {
		...
		return DB::fetch_first("SELECT * FROM %t WHERE praid=%d ", array($this->_table, $id, $idtype));
	}
	public function delete_by_id_idtype($id, $idtype,$uid=0) {
		return DB::delete($this->_table, DB::field('id', $id) .' AND '.DB::field('idtype', $idtype).' AND '.DB::field('uid', $uid));
	}
