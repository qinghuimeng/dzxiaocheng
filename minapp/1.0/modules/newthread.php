...
if(($special == 1 && !$_G['group']['allowpostpoll']) || ($special == 2 && !$_G['group']['allowposttrade']) || ($special == 3 && !$_G['group']['allowpostreward']) || ($special == 4 && !$_G['group']['allowpostactivity']) || ($special == 5 && !$_G['group']['allowpostdebate'])) {
	//howmessage('minbbs_group_nopermission', NULL, array('grouptitle' => $_G['group']['grouptitle']), array('login' => 1));
}
...
if($post_invalid = odz_checkpost($subject, $message, ($special || $sortid))) {
...
if(!$typeid && $_G['forum']['threadtypes']['required'] && !$special && !odz_get_hack('fidcheck', array($_GET['fid']))) {
...
if(array_key_exists('typeoption', $_GET) && !empty($_GET['typeoption']) && !is_array($_GET['typeoption'])) {
	if(ODZ_CHARSET == 'gbk'){
		$_POST['typeoption'] = iconv('gbk', 'utf-8//IGNORE', $_POST['typeoption']);
	}

    $_GET['typeoption'] = json_decode($_POST['typeoption'], true);
    $_GET['typeoption'] = odz_decode($_GET['typeoption']);
//    if($_GET['debug']){die('=========GET: '.serialize($_POST)."\n");}
}
...
if(!empty($_FILES)) {
    ...
    foreach($_FILES as $k => $file) {
        ...
        $_GET['typeoption'][$k] = base64_encode(file_get_contents($file['tmp_name']));
...
if($_G['forum']['threadsorts']['types'][$sortid] && !$_G['forum']['allowspecialonly']) {
	$_G['forum_optiondata'] = odz_threadsort_validator($_GET['typeoption'], $pid);
}
...
$thread['status'] = 32;//设置status 为0时取消回复通知
...
//if($tid&&$_G['fid']==$_G['minbbs_config']['share_fid']){
//    $arr1 = array(
//        'tid' => $tid,
//        'num' => 0,
//    );
//   C::t('home_praise_total')->insert($arr1, true);
//}
...
if($_G['forum']['threadsorts']['types'][$sortid] && !empty($_G['forum_optiondata']) && is_array($_G['forum_optiondata'])) {
	...
	foreach($_G['forum_optiondata'] as $optionid => $value) {
		...
		if($_G['forum_optionlist'][$optionid]['type'] == 'image') {
			...
			$upload = upload_base64_file(unserialize($value));
			$url = $_G['setting']['attachurl'] . 'forum/' .$upload->attach['attachment'];

			$value = serialize(array('aid' => $upload->aid, 'url' => $url));
...
//淮水安澜发帖阳光纪检
odz_get_hack('typehtml', array(&$rushdata));
...
//C::t('forum_post')->update('tid:'.$tid, $pid, array('devicetype' => $devicetype));//发帖插入客户端
DB::query('INSERT INTO '.DB::table('forum_devicetype').' SET pid='.$pid.', devicetype='.$devicetype);
//forumpicstyle反序列化 by zlc
...
//附件目录为相对目录，重新定义为绝对路径，避免调错路径
$_G['setting']['attachurl'] = DISCUZ_ROOT.$_G['setting']['attachurl'];
...
if(isset($_FILES['filedata'])) {
    ...
    if($aids) {
        foreach($aids as $aid) {
            ...
            C::t('forum_post')->update('tid:'.$tid, $pid, array('attachment'=>$attachment));
...
if(($_G['group']['allowpostattach'] || $_G['group']['allowpostimage']) && ($_GET['attachnew'] || $sortid || !empty($_GET['activityaid']))) {
	updateattach_new($displayorder == -4 || $modnewthreads, $tid, $pid, $_GET['attachnew']);
     if(!$threadimageaid) {
...
$result = array(
			'tid'=>$tid,
			'detailurl' => $_G['baseurl'].$_G['minbbs_config']['minbbs_type'].'/index.php?mod=biu_detail&tid='.$tid.'&sharehtml=1&version='.$_G['minbbs_config']['version'],
		 );
...
if($modnewthreads) {
	...
	odz_result($result, odz_lang('need_review'));
} else {
	...
	odz_result($result, 'post_newthread_succeed');
}
?>
